...
Uint32 GetPixel(Bitmap bitmap, int x, int y) {
...
void PutPixel(Bitmap bitmap, int x, int y, Uint32 pixel) {
...
void ReplaceColor(Bitmap bitmap, Uint32 color, Uint32 replace) {
...
void BucketContainer::FindColors(Bitmap bitmap) {
...
BucketContainer::BucketContainer(Bitmap bitmap) {

	FindColors(bitmap);

}

Bitmap RandomizeBitmap(std::string filepath, int weight) {
	/** Load the bitmap */
	Bitmap bitmap = SDL_LoadBMP(filepath.c_str());
	if (bitmap == NULL) {
		return NULL;
	}
